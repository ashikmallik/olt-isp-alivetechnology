...
class Model extends Database
{
    ...
    function view_all_agent_for_excel($table_name, $where_cond)
    {
        ...
        $data = array();
        $sql = "SELECT (SELECT value FROM tbl_setting WHERE other_parameter = 'name') AS Name_of_Operator,
                    ...
                    WHERE ag_status = 1 order by ag_name ASC";
...
public function countOnlineUsers($mikrotikId)
{
    $status = $this->getUserStatus($mikrotikId);
    return $status['online'];
}


public function countOfflineUsers($mikrotikId)
{
    $status = $this->getUserStatus($mikrotikId);
    return $status['offline'];
}

public function countDisabledUsers($mikrotikId)
{
    $status = $this->getUserStatus($mikrotikId);
...
/**
 * Online / Offline / Disabled হিসাব করার মূল কাজ
 */
private function getUserStatus($mikrotikId)
{
    $mk = $this->getSingleData('mikrotik_user', [
        'where' => [
            ['id', '=', $mikrotikId],
            ['status', '=', '1']
        ]
    ...
    if ($mk) {
        ...
        if (!empty($mikrotik) && $mikrotik->connected) {
            ...
            $secrets = $mikrotik->read(false);
            $allSecrets = $mikrotik->parseResponse($secrets);

            // সব active
            $mikrotik->write('/ppp/active/print');
            $active = $mikrotik->read(false);
            $activeUsers = $mikrotik->parseResponse($active);

            if (empty($allSecrets)) {
                return ['online' => 0, 'offline' => 0, 'disabled' => 0];
            }

            // Active list ঠিক করা (pppoe- prefix remove করে)
            $activeList = [];
            if (!empty($activeUsers)) {
                foreach ($activeUsers as $au) {
                    $name = $au['name'];
                    $name = trim(str_replace(['<pppoe-', '>'], '', $name));
                    $activeList[$name] = $name;
                }
            }

            // DB agent list → ip না, name ব্যবহার করলে ভালো হয়
            $agents = $this->getAllData('tbl_agent', [
            ...
            if (!empty($agents)) {
                foreach ($agents as $agent) {
                    // ধরলাম আপনার DB তে agent এর username রাখা আছে ip field এ
            ...
            foreach ($allSecrets as $secret) {
                ...
                if (isset($agentList[$name])) {
                    if ($secret['disabled'] == 'true') {
                        $disabled++;
                    } elseif (isset($activeList[$name])) {
                        $online++;
                    } else {
                        $offline++;
                    }
    ...
    public function updateSecretPassword($mikrotikId, $secretName, $newPassword)
    {
        $mk = $this->getSingleData('mikrotik_user', [
            'where' => [['id', '=', $mikrotikId], ['status', '=', '1']]
        ]);
    ...
    public function Month_Identify($month)
    {
        $months = [
            1 => "January",
            2 => "February",
    ...
    public function previous_due_log_update_or_create($agent_id, $previous_due)
    {
        $check = $this->rawSqlSingle(
            "SELECT id FROM tbl_previous_due
                WHERE tbl_agent_id = $agent_id
        ...
        if ($check) {
            $this->rawSql(
                "UPDATE tbl_previous_due
                SET previous_due_amount = '$previous_due'
                WHERE id = $agent_id"
            );
        } else {
            $this->rawSql(
                "INSERT INTO tbl_previous_due (tbl_agent_id, previous_due_amount)
                        VALUES ($agent_id, $previous_due)"
            );
        }
    ...
    public function packageCreate($package_name, $bill_amount)
    {
        $check = $this->rawSqlSingle(
            "SELECT package_id FROM tbl_package
                WHERE package_name = '$package_name' AND net_speed = '$package_name';"
        );

        if (!$check) {
            $this->rawSql(
                "INSERT INTO tbl_package (package_name, net_speed, bill_amount, type, mikrotik_id)
                        VALUES ('$package_name', '$package_name', '$bill_amount', 1, 1)"
            );
        }
    ...
    public function zoneCreate($zone_name)
    {
        $checkId = $this->rawSqlSingle(
            "SELECT zone_id as zoneId FROM tbl_zone
                WHERE zone_name = '$zone_name';"
        )['zoneId'];

        if (!$checkId) {
            $zoneId = $this->insertData('tbl_zone', [
                'zone_name' => $zone_name,
                'level' => '1',
                'created_by' => 1,
            ]);
        }
        return isset($checkId) ? $checkId : $zoneId;
    }
    ...
    public function createActivityLog(
        ...
        $this->insertData('activity_logs', [
            ...
            'device_info' =>  $device_info ? $this->formatDeviceInfo($_SERVER['HTTP_USER_AGENT']) : null,
    ...
    function formatDeviceInfo($userAgent)
    {
        $os = 'Unknown OS';
