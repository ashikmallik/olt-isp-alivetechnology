...
if (isset($_POST['submit'])) {
    ...
    $form_data_sms = array(
        'smsbody' => $sms_body,
        'smshead' => 'new_customer_sms'
    );


    if (isset($value['status']) && $value['status'] == 4) {
        $smsRow = $obj->Update_data("sms", $form_data_sms, " status='4' ");
    } else {

        $form_data_sms['status'] = 4;

        $smsRow = $obj->insertData("sms", $form_data_sms);
    }

    if ($smsRow) {
        echo '<script> window.location="?page=customize_new_customer_sms"; </script>';
    } else {
                    ...
                    <fieldset class="wizard-fieldset show">
                        <div class="text-center">
                            <div class="col-md-12 bg-teal-800">
                                <h6 class="text-md text-neutral-500">New Custommer SMS</h6>
                            </div>
                            <hr>
