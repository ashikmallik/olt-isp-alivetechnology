...
$value = $obj->details_by_cond("sms", "status='7'");
if (isset($_POST['submit'])) {
    ...
    if (isset($value['status']) && $value['status'] == 7) {
        $smsRow = $obj->Update_data("sms", $form_data_sms, " status='7' ");
    } else {

        $form_data_sms['status'] = 7;

        $smsRow = $obj->insertData("sms", $form_data_sms);
    }

    if ($smsRow) {
        echo '<script> window.location="?page=customize_advance_paid_sms"; </script>';
    } else {
                    ...
                    <fieldset class="wizard-fieldset show">
                        <div class="text-center">
                            <div class="col-md-12 bg-teal-800">
                                <h6 class="text-md text-neutral-500">Customize Advance Paid SMS</h6>
                            </div>
                            <hr>
                        ...
                        <form style="padding: 20px;" role="form" enctype="multipart/form-data" method="post" class="d-flex flex-column align-items-center w-100">
                            <div class="mb-3">
                                ...
                                <strong style="font-size: 14px" class="badge bg-success" title="Total Paid Amount">{ADVANCE_AMOUNT}</strong>
                            ...
                            <div class="form-group w-100">
                                ...
                                <textarea class="form-control" onkeyup="countCharB(this)" name="sms_body" id="ResponsiveDetelis"
                                    rows="7" style="width: 100%;"><?php echo isset($value['smsbody']) ? $value['smsbody'] : NULL; ?></textarea>
                            ...
                            <div style="display: flex; justify-content: center; align-items: center;" class="form-check mt-2 mb-5">
                                ...
                                <label class="form-check-label" style="margin-left: 0px;" for="flexCheckDefault">
                                    Advanced SMS Status
                                    <?php if (isset($value['smshead']) && $value['smshead'] == 'active') { ?>
