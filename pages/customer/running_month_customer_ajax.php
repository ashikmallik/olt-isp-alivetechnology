...
WHERE MONTH(entry_date) = '$runningMonth' AND YEAR(entry_date) = '$runningYear' AND vw_agent.deleted_at is NULL $wherecond ORDER BY $order_by LIMIT $start,$length");

$totalData = count($obj->rawSql("SELECT * FROM vw_agent WHERE MONTH(entry_date) = '$runningMonth' AND YEAR(entry_date) = '$runningYear' AND vw_agent.deleted_at is NULL")); // Total without filters
...
$totalFiltered = $obj->rawSqlSingle(
    ...
    LEFT JOIN _createuser ON vw_agent.billing_person_id = _createuser.UserId 
    LEFT JOIN customer_billing ON vw_agent.ag_id = customer_billing.agid 
    WHERE MONTH(entry_date) = '$runningMonth' AND YEAR(entry_date) = '$runningYear' AND vw_agent.deleted_at IS NULL $wherecond
"
)["total"];
