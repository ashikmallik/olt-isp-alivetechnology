...
$allData = $obj->rawSql("SELECT *,tbl_zone.zone_name,_createuser.FullName FROM vw_agent LEFT JOIN tbl_zone ON vw_agent.zone = tbl_zone.zone_id LEFT JOIN _createuser ON vw_agent.billing_person_id = _createuser.UserId WHERE vw_agent.deleted_at is not NULL $wherecond ORDER BY $order_by LIMIT $start,$length");

$totalData = count($obj->rawSql("SELECT * FROM vw_agent WHERE vw_agent.deleted_at IS NOT NULL")); // Total without filters
$totalFiltered = count($obj->rawSql("SELECT * FROM vw_agent WHERE vw_agent.deleted_at IS NOT NULL")); // Total with applied filters
