...
$PreviousYear = $obj->rawSql("SELECT count(ag_id), MONTH(entry_date) AS month 
                              FROM tbl_agent WHERE YEAR(entry_date)='$previousYear' AND deleted_at is NULL GROUP BY MONTH(entry_date)");

$CurrentYear = $obj->rawSql("SELECT count(ag_id), MONTH(entry_date) AS month 
                             FROM tbl_agent WHERE YEAR(entry_date)='$currentYear' AND deleted_at is NULL GROUP BY MONTH(entry_date)");
