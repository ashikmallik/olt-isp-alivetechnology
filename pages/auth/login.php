...
<style>
    * {
    ...
    @media screen and (min-width: 867px) and (max-width: 1451px) {
        .login-section {
            width: 30%;
        }
    }
...
<div class="login-container">
    ...
    <section class="login-section">
        ...
        <div class="auth-brand text-center text-lg-start">
            <a href="" class="logo logo-dark text-center">
                <span class="logo-lg">
                    <img src="<?php echo $logo ?>" alt="logo" class="logo-lg">
                </span>
            </a>
        </div>
        
        <?php
        $noticeInfo = $obj->rawSqlSingle("SELECT * FROM countdown_notice WHERE id = 1");
        $today = new DateTime();
        $todayDay = (int)$today->format('d');

        $billDate = (int)$noticeInfo['bill_date']; // যেমন: 5
        $displayBefore = (int)$noticeInfo['notice_display_before']; // যেমন: 3

        if ($noticeInfo['forcefully_stop'] == 2 && $noticeInfo['permanently_stop'] == 2) {
            if ($todayDay > $billDate && $noticeInfo['notice_displayable'] == 1) {
                $amethod = "";
            } elseif($noticeInfo['complete_shutdown'] == 1){
                $amethod = "";
            } else {
                $amethod = "POST";
            }
        ...
        ?>
            ...
            <?php
                if($noticeInfo['complete_shutdown'] != 1){
                    if ($noticeInfo['notice_displayable'] == '1' && $noticeInfo['forcefully_stop'] == "2" && $noticeInfo['permanently_stop'] == "2") {
                        $startDay = $billDate - $displayBefore;
                        if ($startDay < 1) $startDay = 1;
        
                        if ($todayDay >= $startDay && $todayDay <= $billDate) {
                            ?>
                                <div class="alert alert-warning text-center" role="alert">
                                    <h6 style="text-decoration: underline;">Notice</h6>
                                    <p><?= htmlspecialchars($noticeInfo['notice']) ?></p>
                                     <p><span style="font-weight: bold; font-size: 20px;" class="text-danger" id="countdown"></span></p>
                                </div>
                                ...
                                <script>
                                    (function() {
                                        ...
                                        // টার্গেট ডেট সেট করা (এই মাসের bill_date দিন)
                                        const targetDate = new Date(today.getFullYear(), today.getMonth(), billDate, 23, 59, 59);
                            ...
                            <?php
                        } else if ($todayDay > $billDate && $noticeInfo['notice_displayable'] == 1) {
                            ?>
                                <div class="alert alert-danger text-center"  role="alert">
                                    <h6 style="text-decoration: underline;">Notice</h6>
                                    <p>Your service is currently inactive due to unpaid dues.
                                        Please clear your outstanding balance to restore your service without interruption.
                                    </p>
                                </div>
                            <?php
                        }
    ...
    <br><br>
    <section class="p-0 m-0 login-logo-parent" style="background-image: url(./assets/images/login-8.png) ; background-size: cover; background-position: center; background-repeat: no-repeat; height: 100vh; width: 100vw;">
        <img src="<?php echo $logo ?>" alt="New ISP" class="img-fluid login-logo">
    </section>
