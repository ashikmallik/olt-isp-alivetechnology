...
$host = $_SERVER['HTTP_HOST']; // e.g. skynetwork.bsdisp.com
$baseUrl = $protocol . $host; // full base URL
...
try {
	...
	foreach ($agentDetail as $agentDetails) {
		...
		$totalDue = intval($obj->rawSqlSingle("SELECT dueadvance FROM customer_billing WHERE agid='$agg_id'")["dueadvance"]);

		$extraRow = '';
		if ($totalDue > 0) {
			$extraRow = '<tr><td>2</td><td>Previous Due</td><td>--</td><td>--</td><td>' . number_format($totalDue > intval($agentDetails["taka"]) ? $totalDue - intval($agentDetails["taka"])  : 0, 2) . '</td></tr>';
		} elseif ($totalDue < 0) {
			$extraRow = '<tr><td>2</td><td>Advanced Payment</td><td>--</td><td>--</td><td>' . number_format(abs($totalDue), 2) . '</td></tr>';
		}
        ...
        <head>
            <style>
                ...
                body {
                font-family: nikosh; /* Use the Nikosh font */
            }
            </style>
        </head>

        <div style="margin-bottom: 10px;">
            <div style="text-align:right; font-weight:bold; color:#2E7D32; font-size: 10pt;">' . $copy . '</div>
            <div style="font-family: Arial; font-size: 7pt; line-height: 1.2;">
                <div style="display: flex; align-items: center; gap: 5px;">
                    <img src="' . $url . $logo . '" width="50" />
                    <div>
                        <div style="font-size: 12pt; font-weight: bold; color:#4CAF50;">' . $CompanyName . '<span style="color:#2E7D32;"></span></div>
                        <div style="font-size: 9pt; color: #666;">' . (isset($address) && $address != '' ? $address : '') . '</div>
                        <div style="font-size: 9pt; color: red;">' . (isset($mobile) && $mobile != '' ? 'Hotline:' . $mobile : '') . '</div>
                        <div style="font-size: 9pt;">' . (isset($email) && $email != '' ? $email : '') . '</div>
                    </div>
                </div>
                ...
                <table width="100%" style="font-size:8.5pt; margin-top:3px;">
                    <tr>
                        <td>
                            ...
                            T.Due: ' . ($totalDue < 0 ? 0 : $totalDue) . ' BDT<br />
                ...
                <table width="100%" border="1" cellspacing="0" cellpadding="3" style="border-collapse:collapse; font-size:8.5pt; margin-top:5px;">
                    ...
                    <tr>
                        ...
                        <td>' . number_format($totalDue < intval($agentDetails["taka"]) ? ($totalDue < 0 ? 0 : $totalDue)  : $agentDetails["taka"], 2) . '</td>
                    </tr>
                    ' . $extraRow . '
                </table>
                ...
                <table width="100%" style="font-size:8.5pt; margin-top:5px;">
                    <tr>
                        <td style="text-align:right;"><b>Total: ' . number_format(($totalDue < 0 ? 0 : $totalDue), 2) . ' BDT</b></td>
                    </tr>
                    <tr>
                        <td style="text-align:right;">In Word: ' . ucwords(convert_number_to_words(($totalDue < 0 ? 0 : $totalDue))) . ' Taka</td>
                    </tr>
                </table>
			...
			if ($copyCount === 1) {
				$bsd .= '<hr style="border: none;border-top: 1px dotted #000; margin: 20px 0;">
	...
	$mpdf = new Mpdf(['orientation' => 'P', 'format' => 'A4', 'default_font' => 'nikosh']);
	$mpdf->WriteHTML($html);
	$mpdf->Output();
