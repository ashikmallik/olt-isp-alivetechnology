            ...
            <div class="form-wizard">
                <fieldset class="wizard-fieldset show">
                    <div class="row gy-3">
                        ...
                        <div class="col-sm-3">
                            <label for="collection-person-filter" class="form-label">Payment Type</label>
                            <div class="position-relative">
                                <select id="billing-paymenttype-filter" class="form-control">
                                    <option value="">All Payment Type</option>
                                        <option value="1">ALL Expence</option>
                                        <option value="3">Bill Colletion</option>
                                        <option value="5">connection Charge or Openning</option>
                                </select></div>
                        </div>
                        <div class="col-sm-3">
                            <label for="collection-person-filter" class="form-label">Payment Method</label>
                            <div class="position-relative">
                                <select id="billing-paymentmethod-filter" class="form-control">
...
<script>
        ...
        var table = $('#account-datatable').DataTable({
            ...
            ajax: {
                ...
                data: function(d) {
                    ...
                    d.typeid = $('#billing-paymenttype-filter').val();
                    d.pay_method = $('#billing-paymentmethod-filter').val();
            ...
            buttons: [
    {
        ...
        messageTop: function() {
            return 'From: ' + $('#date-from').val() + ' To: ' + $('#date-to').val();
        },
        exportOptions: {
            columns: ':visible'
        }
    ...
    },
    {
        ...
        exportOptions: {
            ...
            // lengthMenu: [500, 200, 100, 50, 25],
            order: [[1, 'asc']],
            initComplete: function(settings, json) {
                $('#debit_total').text(json?.totalDebits);
                $('#credit_total').text(json?.totalCredits);
                $('#balance').text(json?.totalCredits - json?.totalDebits);
            },
        ...
        $('#zone-filter, #billing-person-filter, #collection-person-filter, #date-from, #date-to,#billing-paymenttype-filter,#billing-paymentmethod-filter').on('change', function() {
            table.ajax.reload();
        });
    });
</script>
