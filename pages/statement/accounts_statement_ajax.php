...
$pay_method = isset($_GET['pay_method']) ? $_GET['pay_method'] : '';
...
if ($typeid != '') {
    if($typeid==5){
    $wherecond .= " AND tbl_account.acc_type IN(4,5)";
    }elseif($typeid==1){
        $wherecond .= " AND tbl_account.acc_type IN(1,22)";
    }
    else{
    $wherecond .= " AND tbl_account.acc_type = '$typeid'";
    }
}
if ($pay_method != '') {
    $wherecond .= " AND tbl_account.payment_method = '$pay_method'";
}
...
$totalCredits = $obj->rawSqlSingle("SELECT SUM(tbl_account.acc_amount) AS totalCredits FROM tbl_account LEFT JOIN tbl_agent ON tbl_agent.ag_id = tbl_account.agent_id WHERE tbl_agent.deleted_at IS NULL AND acc_type != 1 $wherecond")["totalCredits"];
$totalDebits = $obj->rawSqlSingle("SELECT SUM(tbl_account.acc_amount) AS totalDebits FROM tbl_account LEFT JOIN tbl_agent ON tbl_agent.ag_id = tbl_account.agent_id WHERE tbl_agent.deleted_at IS NULL AND acc_type = 1  $wherecond")["totalDebits"];
...
foreach ($allData as $accounts) {
    ...
    if ($accounts['acc_type'] == 1 || $accounts['acc_type'] == 22) {  // Assuming 1 is for debit
        $debit = $amount;
        $balance -= $debit;
        // $debit_total += $debit;
    ...
    $cus_id_info = isset($accounts['cus_id']) ? ' (ID: ' . $accounts['cus_id'] . ')' : '';
    $cus_IP_info = isset($accounts['ip']) ? ' (IP: ' . $accounts['ip'] . ')' : '';
    $accounts['acchead'] = ($accounts['agent_id'] > 0) ? $accounts['ag_name'] . $cus_id_info . $cus_IP_info : $accounts['acc_name'];
...
echo json_encode([
    ...
    "recordsTotals" => $totalData,    // Total records in the database
    "recordsFiltereds" => $totalFiltered, // Filtered records count
    ...
    "totalCredits" => intval($totalCredits),
    "totalDebits" => intval($totalDebits),
], JSON_PRETTY_PRINT);
exit();
?>
